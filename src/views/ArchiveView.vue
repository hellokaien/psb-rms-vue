...
<template>
  <AdminShell active-route="archive" title="Archive" subtitle="Review and restore closed referral records">
    <section class="mb-5 grid grid-cols-2 gap-3 lg:grid-cols-4">
      <article class="stat"><span>Archived Records</span><strong>{{ archived.length }}</strong><small>Demo records
          available</small></article>
      <article class="stat"><span>Archived This Year</span><strong>{{archived.filter((item) =>
        item.archived.startsWith('2026')).length }}</strong><small>Since January 2026</small></article>
      <article class="stat"><span>Retention Period</span><strong>7 years</strong><small>Configured in Settings</small>
      </article>
      <article class="stat"><span>Storage Used</span><strong>1.8 GB</strong><small>32% of archive allocation</small>
      </article>
    </section>
    <section class="filters"><label class="search">
        <AppIcon name="search" class="h-4 w-4 text-gray-400" /><input v-model="searchQuery" type="search"
          placeholder="Search archived referrals...">
        ...
        <table class="w-full min-w-[820px]">
          ...
          <tbody>
            <tr v-for="item in filteredItems" :key="item.id">
              ...
              <td class="font-medium text-gray-700">{{ item.client }}</td>
              <td>{{ item.type }}</td>
              <td>{{ item.closed }}</td>
              <td>{{ item.archived }}</td>
              <td>{{ item.outcome }}</td>
              <td class="text-right"><button class="mr-3 font-medium text-[#c9a83e]" type="button"
                  @click="selectedItem = item">View</button><button class="font-medium text-blue-600" type="button"
                  @click="restore(item)">Restore</button></td>
            </tr>
            <tr v-if="!filteredItems.length">
              <td colspan="7" class="py-10 text-center text-gray-400">No archived records found.</td>
            </tr>
          </tbody>
        </table>
      </div>
      <div class="footer">Showing {{ filteredItems.length }} of {{ archived.length }} archived records</div>
    </section>
    <div v-if="selectedItem" class="overlay" @click.self="selectedItem = null">
      <div class="modal">
        <div class="mb-4 flex items-center justify-between">
          <div>
            <h2 class="text-lg font-semibold text-gray-800">{{ selectedItem.id }}</h2>
            <p class="text-xs text-gray-400">Archived referral details</p>
          </div><button class="text-2xl text-gray-400" @click="selectedItem = null">×</button>
        </div>
        <dl>
          <div>
            <dt>Client</dt>
            <dd>{{ selectedItem.client }}</dd>
          </div>
          <div>
            <dt>Case Type</dt>
          ...
          <div>
            <dt>Outcome</dt>
            <dd>{{ selectedItem.outcome }}</dd>
          </div>
          <div>
            <dt>Closed Date</dt>
            <dd>{{ selectedItem.closed }}</dd>
          </div>
          <div>
            <dt>Archived Date</dt>
            <dd>{{ selectedItem.archived }}</dd>
          </div>
          <div>
            <dt>Archived By</dt>
            <dd>{{ selectedItem.archivedBy }}</dd>
          </div>
        </dl>
        <div class="mt-5 flex flex-wrap justify-end gap-2"><button class="danger"
            @click="permanentlyDelete(selectedItem)">Delete Permanently</button><button class="secondary"
            @click="selectedItem = null">Close</button><button class="primary" @click="restore(selectedItem)">Restore
            Referral</button></div>
      </div>
    </div>
